...
import {
  useEventThreads, createThread, type ThreadWithActor,
} from '@/hooks/use-event-threads'
import {
  visibleChannels, deriveAudience, canAsk,
...
export function DiscussionFestival({ event }: { event: { id: string; name?: string; participant_count?: number } }) {
  const { currentActor, person, entities } = useAuth()
...
function ThreadCard({ thread, open, onToggle, onChanged }: {
